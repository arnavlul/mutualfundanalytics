...
pd.DataFrame(returns_computed).to_csv(os.path.join(BASE_DIR, 'data', 'processed', 'returns_computed.csv'), index=False)
...
cagr_df.to_csv(os.path.join(BASE_DIR, 'data', 'processed', 'cagr_report.csv'), index=False)
...
sharpe_df.to_csv(os.path.join(BASE_DIR, 'data', 'processed', 'sharpe_values.csv'), index=False)
...
pd.DataFrame(sortino_data).to_csv(os.path.join(BASE_DIR, 'data', 'processed', 'sortino_values.csv'), index=False)

# Task 5: Alpha & Beta vs benchmark (Nifty 100)
print("Task 5: Computing Alpha & Beta...")
benchmarks = pd.read_csv(os.path.join(BASE_DIR, 'data', 'processed', 'clean_benchmark_indices.csv'))
...
alpha_beta_df.to_csv(os.path.join(BASE_DIR, 'data', 'processed', 'alpha_beta.csv'), index=False)
...
max_dd_df.to_csv(os.path.join(BASE_DIR, 'data', 'processed', 'max_drawdown.csv'), index=False)
...
metrics.to_csv(os.path.join(BASE_DIR, 'data', 'processed', 'fund_scorecard.csv'), index=False)
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(BASE_DIR, 'reports', 'benchmark_chart.png'))
plt.close()
...
with open(os.path.join(BASE_DIR, 'notebooks', '04_performance_analytics.ipynb'), 'w') as f:
    nbf.write(nb, f)
